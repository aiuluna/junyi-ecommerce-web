...
LQ.selectUi={
	show:function(options){
		...
		/*是否传值过来*/
		var $val=$this.children(".selectfocus").attr("data-value");
	...
	selectFun:function(obj,value){
		...
		var $field = $("#" + id);
		$field.show();
		$field.children('.selectfocus').html('<em>'+obj['selectTxt']+'</em>');
		$field.children('input[type="hidden"]').val('');
		LQ.selectUi.appendData(id,value);
		$field.find("li").bind("click",function(e){
			e.stopPropagation();
			$(this).parent().siblings('.selectfocus').html('<em>'+$(this).text()+'</em>');
			$(this).parent().siblings('input[type="hidden"]').val($(this).attr("data-val"));
    ...
    if(value){
			if(typeof(value)=='object'){
				...
				$field.children("select").empty();
				$field.children("select").append(option);
				$field.children("ul").empty();
				$field.children("ul").append(listr);
			}
		}
		
		var ul=$field.children("ul");
		if(items>10){
			ul.height(200);
		}else{
			ul.height('');
