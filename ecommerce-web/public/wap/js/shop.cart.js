$(function() {
  ...
  if (typeof _ERR_MSG === 'string') ShopPopup.alert(_ERR_MSG, null, '错误');
});
...
var CartManager = {
  ...
  submitPreOrder: function(items) {
    ...
    ShopPopup.popupLoading('页面跳转中...'); // 无论接下来是成功还是失败，都不需要关闭这个popup
    // 判断用户是否已经登录
    var self = this;
    $.ajax({
      method: 'POST',
      ...
      self.submitInProgress = false;
      window.location.href = 'passport-login.html?redirectUrl=cart.html';
    });
  },
...
function CartCtrl(cartElement) {
  ...
  // init tr
  cartEl.find('li.goodsline').each(function() {
...
$.extend(CartCtrl.prototype, {
  ...
  notifyRowDeleted: function(rowCtrl) {
    ...
    if (rowCtrl.orderActId) {
      ...
      if (removeOrderActRow) {
        this.cartEl.find('.order-act-row.act-' + rowCtrl.orderActId).remove();
      }
    }
  ...
  _updateRecalcResult: function(data) {
    ...
    if (data.errorMessage) {
      ...
      if (msg.indexOf('，')) {
        ...
        var slen = Math.floor(msg.length / 2);
        msg = msg.substr(0, slen) + '<br/>' + msg.substr(slen);
      }
      this.spanErrorMessage.html(msg + '<i></i>').show();
    } else {
    ...
    if (data.orderActList) {
      ...
      data.orderActList.forEach(function(o) {
        ...
        el.parent().show().siblings('.coudan').hide();
      });
    }
    // 判断是否有单个商品违规错误
  ...
  _getSelectedItems: function() {
    var cartRowMap = this.cartRowMap,
      items = [];
    this.cartEl.find('li.goodsline').each(function() {
...
function CartRowCtrl(cartCtrl, tr) {
  ...
  rowEl.find('.goodsDel').click(function() {
    // 跳出confirm窗口确认
    ShopPopup.confirm('确定要移除该商品吗？', function(result) {
...
$.extend(CartRowCtrl.prototype, {
  ...
  _initQuantityEditCtrl: function() {
    var rowEl = this.rowEl,
      btnRedu = this.btnRedu = rowEl.find('.dt-redu'),
      btnAdd = this.btnAdd = rowEl.find('.dt-add'),
      inputQuantity = this.inputQuantity = rowEl.find('.dt-count'),
      self = this;
    ...
    this.stock = Number(rowEl.find('input[name=stock]').val());
    this.restrictNumber = Number(rowEl.find('input[name=restrictNumber]').val());
    this.msgEl = rowEl.find('.stock-message');
...
function popupSevereError(message) {
  ...
  hasSevereError = true;
  ShopPopup.alert(message, function() {
    window.location.reload();
  }, '错误');
}

function ajaxErrorHandler() {
  ShopPopup.alert('服务器出现意外错误，请稍后重试');
}